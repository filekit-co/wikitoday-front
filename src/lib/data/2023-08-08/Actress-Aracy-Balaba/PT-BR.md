---
title: 'A atriz Aracy Balabanian morre aos 83 anos: Celebridades prestam homenagens'
description: 'As celebridades usaram as mídias sociais para prestar homenagem à atriz, que morreu na segunda-feira (7).'
category: 'Entertainment'
keywords: 'Aracy Balabanian morreu, Sai de Baixo, Atriz Aracy, atriz Aracy Balabanian, Miguel Falabella, morre aracy balabanian, aracy atriz, aracy balabanian sai de baixo, morte de Aracy Balabanian, aracy balabanian filhos, atriz Aracy Balabanian morreu, Claudia Raia'
date: '2023-08-08'
author: 'wikitoday.io'
language: 'PT-BR'
thumbnail: 'https://s2-g1.glbimg.com/u141iBnwhrfuORATs1tb6rnODYc=/1200x/smart/filters:cover():strip_icc()/i.s3.glbimg.com/v1/AUTH_59edd422c0c84a879bd37670ae4f538a/internal_photos/bs/2020/X/s/fbcrSXRpe0WvBEdCCAgA/assets-fotos-483-185166.jpg'
---

# A atriz Aracy Balabanian morre aos 83 anos: Celebridades prestam homenagens

<p class="datetime"><em>2023-08-08 - wikitoday<em></p>

<blockquote class="quote-container dark">
  <p class="quote-text dark">
    As celebridades usaram as mídias sociais para prestar homenagem à atriz, que morreu na segunda-feira (7).
  </p>
</blockquote>


<figure class=image-container>
    <img src="https://s2-g1.glbimg.com/u141iBnwhrfuORATs1tb6rnODYc=/1200x/smart/filters:cover():strip_icc()/i.s3.glbimg.com/v1/AUTH_59edd422c0c84a879bd37670ae4f538a/internal_photos/bs/2020/X/s/fbcrSXRpe0WvBEdCCAgA/assets-fotos-483-185166.jpg" alt="G1" />
    <figcaption>
        <h4> from G1</h4>
    </figcaption>
</figure>


<hr class="article-hr" />


<div class="faq">

<details class="group" open>
  <summary class="summary">
    <h2><b>Q. Quantos anos tinha Aracy Balabanian quando morreu?</b></h2>
    <span class="icon-container">
      <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-closed" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/>
      </svg>
      <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-open" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/>
      </svg>
    </span>    
  </summary>
  <p>Aracy Balabanian morreu aos 83 anos de idade.</p>
</details>

<details class="group" >
  <summary class="summary">
    <h2><b>Q. Por que Aracy Balabanian era conhecida?</b></h2>
    <span class="icon-container">
      <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-closed" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/>
      </svg>
      <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-open" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/>
      </svg>
    </span>    
  </summary>
  <p>Aracy Balabanian ficou conhecida por seu papel no programa &#39;Sai de Baixo&#39;, da TV Globo.</p>
</details>

<details class="group" >
  <summary class="summary">
    <h2><b>Q. Quem prestou homenagem a Aracy Balabanian?</b></h2>
    <span class="icon-container">
      <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-closed" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/>
      </svg>
      <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-open" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/>
      </svg>
    </span>    
  </summary>
  <p>Celebridades como Miguel Falabella, Claudia Raia, Beth Goulart e Patrícia Pillar prestaram homenagem a Aracy Balabanian.</p>
</details>

<details class="group" >
  <summary class="summary">
    <h2><b>Q. Onde será realizado o funeral?</b></h2>
    <span class="icon-container">
      <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-closed" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/>
      </svg>
      <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-open" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/>
      </svg>
    </span>    
  </summary>
  <p>O velório será realizado no Theatro Municipal do Rio.</p>
</details>

<details class="group" >
  <summary class="summary">
    <h2><b>Q. O público poderá comparecer ao velório?</b></h2>
    <span class="icon-container">
      <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-closed" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/>
      </svg>
      <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-open" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/>
      </svg>
    </span>    
  </summary>
  <p>Sim, o velório será aberto ao público no Theatro Municipal do Rio.</p>
</details>

</div>


<hr class="article-hr" />

<div class="article-body">
A atriz Aracy Balabanian morreu na manhã desta segunda-feira (7), aos 83 anos, no Rio de Janeiro. A confirmação foi feita pela Clínica São Vicente, na Gávea, zona sul da cidade. A causa da morte não foi revelada.


</div>


<figure class=image-container>
    <img src="https://imagens.ebc.com.br/78JQXNO4b4oO_qc5mLsnW_tY3Rk=/1600x800/https://agenciabrasil.ebc.com.br/sites/default/files/thumbnails/image/_leandra_leal_and_aracy_balabanian_03_jv.jpg?itok=-1GYFOLI" alt="Agência Brasil" />
    <figcaption>
        <h4> from Agência Brasil</h4>
    </figcaption>
</figure>


<div class="article-body">
Balabanian teve uma carreira de mais de 50 anos, com participação em mais de 30 novelas e peças de teatro. Ela era amplamente conhecida por seu papel no programa &#39;Sai de Baixo&#39;, da TV Globo. As celebridades usaram as redes sociais para expressar suas condolências e prestar homenagem à atriz icônica.

 Miguel Falabella, seu colaborador de longa data, escreveu um tributo sincero, chamando-a de grande atriz e profissional impecável. Outras celebridades, como Claudia Raia, Beth Goulart e Patrícia Pillar, também se despediram de Balabanian e elogiaram seu talento e suas contribuições para a televisão brasileira. O velório será realizado no Theatro Municipal do Rio, e a atriz será cremada em cerimônia fechada para amigos e familiares no Cemitério do Caju.


</div>